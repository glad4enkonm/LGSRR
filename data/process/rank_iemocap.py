...
def rank_response(sentence,words,turn):
...
def generate_rank(num,writer,start=0):
    lines=[]
    lines_rank=[]
    # train_desc.tsv is the combination of train.tsv(the original data) and videollama2_iemocap-da.tsv(the description data generated in CoT-Step 2).
    # for each video, train_desc.tsv contains the turn, intent, text and three generated descriptions in order.
    desc_path="data/data_desc/iemocap_da/train.tsv"
    ...
    for i in tqdm(range(start,num)):
        ...
        line_rank=rank_response(response,["Speakers' Actions","Facial Expressions","Interaction with Others"],turn)
        line_rank.insert(0,turn)
        lines_rank.append(line_rank)
        writer.writerow(line_rank)

with open("data/data_rank/rank_iemocap_train.tsv",'w',newline='', encoding='utf-8') as file:
    writer=csv.writer(file,delimiter='\t')
    writer.writerow(["Turn","Rank_First","Rank_Second","Rank_Third"])
    num = 6590 # numbers of samples in the training set
